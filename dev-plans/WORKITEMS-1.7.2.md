...
Scope rule: every workitem below either (a) closes a TEST-AUDIT-1.6.1 blind spot, (b) lifts a quality-scale rule from `todo`/missing to `done`/`exempt`, or (c) hardens the gate around (a)+(b) so the claim survives future drift. Nothing else lands in 1.7.2 — keep the file small and the gate green.
