...
- [ ] 34 Move the now/scheduled radio buttons below the version and target worker selection. 
- [ ] 35 The Devices tab on the HA column: are we able to reliably link to the Devices page back in Home Assistant from here? If yes, please add that link for devices that are registered in Home Assistant. 
- [ ] 36 The "Schedule Upgrade" button from the hamburger menu, please. 
- [ ] 37 The "Remove schedule" button on the schedule page, as well as the "Remove schedule" button in the upgrade dialog, both don't actually remove the schedule. Nor does the button at the top of the devices page. 
- [ ] 38 In the version drop-down on the upgrade dialog, let's change the text from "latest" to "current" for the current ESPHome version, as "latest" is not technically correct. 
