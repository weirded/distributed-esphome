...
- [ ] 22 Speaking of this more, let's actually simplify the user interface instead of having an upgrade modal and a schedule upgrade modal. Let's fold the scheduling piece into the upgrade modal as an optional thing to enable. Do this both for the row-level upgrade as well as the upgrade options at the top. What will mean we'll need to have an edit scheduled upgrade button as well. Again, in that we should show more or less the same dialogue that lets you pick the ESP home version as well as the slave to run or the worker to run on. For the ESP home version, one of the options should be "current version". 
