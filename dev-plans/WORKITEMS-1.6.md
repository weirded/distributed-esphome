...
- [ ] **JH.3 Retention** — new add-on option `job_history_retention_days` (default 365, max 3650, 0 = unlimited). Nightly background task in `main.py` deletes rows older than the retention window. Log the evict count at INFO. Even at hundreds of compiles/day, the DB stays well under 100 MB at a year of retention.
...
- [x] **#82** *(1.6.0-dev.1)* — **macOS workers broken by SC.3 hash-pinned install.** After #80 fixed the `-c`/`-r` bug, macOS workers (e.g. `AI-MacBook-Pro.localdomain`, `macdaddy.localdomain`) still failed every `esphome` compile in 5–6 seconds, intermittently breaking the `compile runs to completion and live log streams` hass-4 smoke test when a macOS worker claimed the job first. Root cause: the committed `esphome-constraints/<version>.txt` files are generated by `pip-compile --generate-hashes --strip-extras` in `python:3.11-slim` (linux/amd64). Platform-conditional transitives like `bleak → pyobjc-core>=10.3` (gated on `platform_system == "Darwin"`) don't appear in the Linux-resolved graph, so `pyobjc-core` is never pinned in the committed `.txt`. Running `pip install --require-hashes -r <file>` on macOS then fails with `ERROR: In --require-hashes mode, all requirements must have their versions pinned with ==. These do not: pyobjc-core>=10.3 from … (from bleak==2.1.1->…)`. Fix: `_constraints_for()` short-circuits to `None` on non-Linux, falling into the existing WARN+unpinned path (the pre-SC.3 behavior macOS/Windows workers had anyway). Linux workers — the primary deployment target — still get hash-pinned installs. Scope documented in the module docstring and `ha-addon/client/esphome-constraints/README.md`. Verified: macOS 14 / arm64 worker now installs 2026.4.0 unpinned; hass-4 smoke suite passed 9/9 on 1.6.0-dev.1.
