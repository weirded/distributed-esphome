...
160. OPEN - OTA diagnostics reports wrong device name. (GitHub issue #15) After OTA failure, network diagnostics shows "neopixel.local" instead of the actual target device. Root cause: `_ota_network_diagnostics()` in client.py uses a naive regex to find `name:` in the YAML, which matches the first `name:` key in any component (e.g. a neopixel light) rather than `esphome.name`. Fix: stop hand-parsing YAML with regex entirely — use ESPHome's own YAML resolution (or at minimum, only look under the `esphome:` block). This is the same class of bug as the fragile regex parsing flagged in LIB.3.
